...
if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1) {
	...
	sprintf(temp2,",%d,%u,%c",HWaddr->sll_ifindex,HWaddr->sll_hatype,HWaddr->sll_halen);
...
int main(int argc, char **agrv){
	struct sockaddr * ipaddr = malloc(sizeof(struct sockaddr));
	ipaddr->sa_family=1;
