...
int AREQresponse(int table, int halen, int waiting)
{
	...
	hw.sll_addr[6] = 0x00;
	hw.sll_addr[7] = 0x00;
	err_msg("%s",arp_table[t].ip_addr);
	char * last=	strrchr(arp_table[t].ip_addr, '.');
	last=(char *)(last+1);
	char * lastcan=	strrchr(	arp_table[0].ip_addr, '.');
	lastcan=(char *)(lastcan+1);
	int lastval=atoi(last);
	int lastcanval=atoi(lastcan);
	int diff = lastval-lastcanval;
	memcpy((void*)arp_table[t].if_haddr, (void*)arp_table[0].if_haddr, 6);
	arp_table[t].if_haddr[5]=arp_table[0].if_haddr[5]+diff;
	hw.sll_ifindex = htons(arp_table[t].if_index);
	hw.sll_halen = halen;
	hw.sll_hatype = arp_table[t].sll_hatype;
	memcpy((void*)hw.sll_addr, (void*)arp_table[t].if_haddr, 6);
...
int processAREQ(char* rcvline)
{
	...
	else
	{
		...
		sendRequest(address);
		AREQresponse(lastentry, halen, 1);
	}
